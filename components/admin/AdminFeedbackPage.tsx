...
const AdminFeedbackPage: React.FC<Props> = ({ user }) => {
  ...
  const refreshData = () => {
    // Only show ADMIN targeted feedbacks
    setFeedbacks(getStudentFeedbacks('ADMIN'));
  ...
  const handleReplySubmit = () => {
    ...
    if (replyTarget.type === 'FEEDBACK') {
      replyToFeedback(replyTarget.id, replyText, user.id);
    } else {
      replyToMaintenance(replyTarget.id, replyText, user.id);
    }
    setReplyTarget(null);
    setReplyText('');
    refreshData();
  ...
  const handleResolve = (id: string, type: 'FEEDBACK' | 'MAINTENANCE') => {
    if (window.confirm("Mark this item as resolved? The submitter will be notified.")) {
      if (type === 'FEEDBACK') resolveFeedback(id, user.id);
      else resolveMaintenance(id, user.id);
      refreshData();
    }
  };

  const handleArchive = (id: string, type: 'FEEDBACK' | 'MAINTENANCE') => {
    if (window.confirm("Archive this ticket? It will be hidden from the main view.")) {
      if (type === 'FEEDBACK') archiveFeedback(id);
      else archiveMaintenance(id);
      refreshData();
    }
  };
