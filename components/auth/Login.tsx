...
const Login: React.FC<LoginProps> = ({ onLogin, isDarkMode, toggleTheme }) => {
    ...
    const [isResetMode, setIsResetMode] = useState(false); // NEW: Toggle state
    const [resetMessage, setResetMessage] = useState(''); // NEW: Success message state
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (isResetMode) {
            ...
            try {
                ...
                setError('Failed to send reset email. Ensure this email is registered.');
                setIsLoading(false);
            }
            return; // Stop here, do not run login logic
        }
                            ...
                            <>
                                ...
                                <div className="bg-white dark:bg-slate-800 py-8 px-4 shadow-xl shadow-slate-200/50 dark:shadow-none sm:rounded-2xl sm:px-10 border border-slate-100 dark:border-slate-700">
                                    ...
                                    <form className="space-y-6" onSubmit={handleSubmit}>
                                        ...
                                        {resetMessage && (
                                            <div className="bg-green-50 dark:bg-green-900/30 border border-green-200 dark:border-green-800 text-green-700 dark:text-green-300 px-4 py-3 rounded-lg text-sm flex items-center gap-2">
                                                <i className="fa-solid fa-check-circle"></i> {resetMessage}
                                            </div>
                                        )}
