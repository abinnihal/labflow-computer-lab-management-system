                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filteredResources.length === 0 ? (
                        ...
                        filteredResources.map(file => {
                            // --- DYNAMIC ICON CHECK ---
                            const isImage = file.type?.toUpperCase() === 'IMAGE' || file.url.match(/\.(jpeg|jpg|gif|png)$/i);
                            const iconClass = isImage ? "fa-solid fa-image" : "fa-solid fa-file-pdf";
                            const bgClass = isImage ? "bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400" : "bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400";
                            ...
                            return (
                                <div key={file.id} className="bg-white dark:bg-slate-800 rounded-xl p-5 shadow-sm border border-slate-200 dark:border-slate-700 hover:shadow-md transition-all group">
                                    <div className="flex justify-between items-start mb-4">
                                        <div className={`w-12 h-12 rounded-lg flex items-center justify-center text-2xl group-hover:scale-110 transition-transform ${bgClass}`}>
