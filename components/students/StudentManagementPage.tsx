...
const StudentManagementPage: React.FC<Props> = ({ user }) => {
  ...
  const handleSaveEdit = async () => {
    if (editingStudent && editForm.name) {
      await updateUser(editingStudent.id, editForm);
  ...
  const handleSaveLogEdit = async () => {
    if (editingLog) {
      await updateAttendanceRecord(editingLog.id, {
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="text-center py-20 text-slate-500">
          <i className="fa-solid fa-circle-notch fa-spin mr-2"></i> Loading student data...
        </div>
      ) : (
        <>
          {activeTab === 'ROSTER' && (
